...
def calculate_depth(points):
    depths = torch.linalg.vector_norm(points, dim=2)
    max_depths = depths.max(dim=1, keepdim=True)[0]
    min_depths = depths.min(dim=1, keepdim=True)[0]
    depths = (depths - min_depths) / (max_depths - min_depths)
    return depths
...
class UniformDropout(nn.Module):
    ...
    def forward(self, x):
        ...
        dropout_mask = random_tensor > self.base_p
        return x * dropout_mask.float()

# Depth-Aware Dropout Rate
...
# Enum for dropout type
class DropoutType(Enum):
    NO_DROPOUT = 1
    UNIFORM_MC_DROPOUT = 2
    DEPTH_AWARE_MC_DROPOUT = 3

# Segmentation head that combines no dropout, mc dropout, and depth-aware mc dropout.
class PointNetMCDropSegHead(nn.Module):
    def __init__(self, dropout_type, num_points, num_global_feats, m, base_p, max_p):
        super(PointNetMCDropSegHead, self).__init__()
        ...
        self.bn2 = nn.BatchNorm1d(256)
        self.bn3 = nn.BatchNorm1d(128)
        if dropout_type == DropoutType.UNIFORM_MC_DROPOUT:
            self.dropout = UniformDropout(base_p)
        elif dropout_type == DropoutType.DEPTH_AWARE_MC_DROPOUT:
            self.dropout = DepthAwareDropout(base_p, max_p)

    def forward(self, x):
        if self.dropout_type == DropoutType.DEPTH_AWARE_MC_DROPOUT:
            depth = calculate_depth(x)
        x = x.transpose(2, 1)
        x, crit_idxs, A_feat = self.backbone(x)
        x = self.bn1(F.relu(self.conv1(x)))
        if self.dropout_type == DropoutType.DEPTH_AWARE_MC_DROPOUT:
            x = self.dropout(x, depth)
        elif self.dropout_type == DropoutType.UNIFORM_MC_DROPOUT:
            x = self.dropout(x)
        x = self.bn2(F.relu(self.conv2(x)))
        x = self.bn3(F.relu(self.conv3(x)))
        x = self.conv4(x)
